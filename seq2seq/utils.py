...
def make_positions(tensor, padding_idx, left_pad, onnx_trace=False):
    ...
    if not hasattr(make_positions, 'range_buf'):
        # 创建一个type device都跟tensor一样的向量，且这个向量没内容
        make_positions.range_buf = tensor.new()
    make_positions.range_buf = make_positions.range_buf.type_as(tensor)
    if make_positions.range_buf.numel() < max_pos:
        # 得到从pad_idx+1开始，到max_pos的tensor
        torch.arange(padding_idx + 1, max_pos, out=make_positions.range_buf)
    mask = tensor.ne(padding_idx)
    # 将position拓展成与postions一致
    positions = make_positions.range_buf[:tensor.size(1)].expand_as(tensor)
    if left_pad:
        positions = positions - mask.size(1) + mask.long().sum(dim=1).unsqueeze(1)
    # positions[mask]将非pad的position选出来
    # tensor.clone()复制原本的tensor，用计算好的position代替原本的token，pad元素保持不变
